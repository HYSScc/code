...
//        NSDictionary *dic5 = @{@"name" : @"wangba", @"desc" : @"王八1"};
        _imageList = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"imageList.plist" ofType:nil]];
    }
    return _imageList;
}
...
- (UILabel *)nameLabel
{
    if (_nameLabel == nil) {
        ...
        _nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, nameY, self.view.bounds.size.width, 100)];
        _nameLabel.text = @"123";
...
//    }
    ...
    self.nameLabel.text = self.imageList[self.index][@"desc"];
