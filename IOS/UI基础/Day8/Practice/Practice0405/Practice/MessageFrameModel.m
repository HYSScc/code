...
- (void)setMessageModel:(MessageModel *)messageModel
{
    ...
    CGFloat textViewBtnY = CGRectGetMaxY(_timeF) + xPadding;
    ...
    CGSize btnSize = CGSizeMake(textViewBtnRealSize.width + 40, textViewBtnRealSize.height + 40); // 添加
    
    if (messageModel.type == MessageModelG) {
        textViewBtnX = xScreenWidth - xIconWidth - 2*xPadding - btnSize.width;
    }else
    {
        textViewBtnX = 2*xPadding + xIconWidth;
    }
    self.textViewBtnF = (CGRect){{textViewBtnX, textViewBtnY}, btnSize};
...
+ (NSMutableArray *)messageFrameModel
{
    ...
    for (NSDictionary *dict in array) {
        ...
        // 隐藏时间。进行比较，
        mm.hideTime = [mm.time isEqualToString:lastMessageFrameModel.messageModel.time];
