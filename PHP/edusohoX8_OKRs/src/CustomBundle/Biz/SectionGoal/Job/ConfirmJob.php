...
class ConfirmJob extends BaseJob implements Job
{
    public function execute($params)
    {
        $userSectionGoal = $this->getUserSectionGoalService()->findAllNotConfirmBySectionGoalId($params['id']);
        ...
        if ($params['beforeDays'] == 'one') {
            foreach ($users as $user) {
                ...
                $this->sendMail($users, $params, $managers);
            }
    ...
    private function getOrgManagers($orgIds)
    {
        ...
        $orgManagers = ArrayToolkit::column($org, 'managerIds');
        $managers = array();
        foreach ($orgManagers as $orgManager) {
            $ids = explode('.', $orgManager);
