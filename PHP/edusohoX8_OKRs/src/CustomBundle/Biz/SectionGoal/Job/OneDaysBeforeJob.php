...
class OneDaysBeforeJob extends BaseJob implements Job
{
    protected $size = 100;

    public function execute($params)
    {
        $params['beforeDays'] = 'one';
    ...
    protected function addConfirmJob($params)
    {
        try {
            ...
            $job = array(
                'name' => 'ConfirmJob',
                'cycle' => 'once',
                'nextExcutedTime' =>  strtotime(date('Y-m-d', strtotime('-1 day', $params['confirmEndTime']))),
    ...
    protected function addSelfRatedJob($params)
    {
        try {
            ...
            $job = array(
                'name' => 'SelfRatedJob',
                'cycle' => 'once',
                'nextExcutedTime' => strtotime(date('Y-m-d', strtotime('-1 day', $params['selfRatingEndTime']))),
    ...
    protected function addOtherRatedJob($params)
    {
        try {
            ...
            $job = array(
                'name' => 'OtherRatedJob',
                'cycle' => 'once',
                'nextExcutedTime' => strtotime(date('Y-m-d', strtotime('-1 day', $params['othersRatingEndTime']))),
