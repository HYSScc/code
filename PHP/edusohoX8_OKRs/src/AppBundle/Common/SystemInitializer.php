...
class SystemInitializer
{
    ...
    public function init()
    {
        ...
        $this->_initLdapUser();
        $this->_initEmailSetting();
    }

    protected function _initPost()
    {
        $this->output->write('  初始化岗位  ');

        $this->getPostService()->addPost(array(
            'code' => '001',
            'name' => '部门经理',
            'createdTime' => time(),
            'updatedTime' => time(),
    ...
    private function _initScoreSetting()
    {
        $this->output->write('  初始化评分设置');

        $default = array();
        $count = 10;
        for ($index = 0; $index < $count; ++$index) {
            $num = $index * 0.1 + 0.1;
            $default['num'.$index] = array($num.'', $num);
        }

        $this->getSettingService()->set('score', $default);

        $this->output->writeln(' ...<info>成功</info>');
    }

    private function _initEmailSetting()
    {
        $this->output->write('  初始化邮箱设置');

        $default = array(
        'email' => 'test@edusoho.com',
        'password' => 'Kzwll123!@#',
        'confirmWarnTitle' =>'确认时间截止提醒',
        'confirmWarnBody' =>'Hi,*nickname*。您的阶段性目标*sectionGoal*未确认,请尽快确认！（这是一封自动产生的email，请勿回复。）',
        'othersRatingWarnTitle' =>'他评提醒',
        'othersRatingWarnBody' =>'Hi,*nickname*。您未对*Rating*的阶段性目标*sectionGoal*进行他评,请尽快前往完成评价！传送门：*sectionGoaUrl*[如果以上链接无法点击,请将上面的地址复制到你的浏览器(如IE)的地址栏中打开.]（这是一封自动产生的email，请勿回复。）',
        'selfRatingWarnTitle' =>'自评提醒',
        'selfRatingWarnBody' =>'Hi,*nickname*您的阶段性目标*sectionGoal*未进行自评,请尽快前往自评并邀请他人评价！传送门：*sectionGoaUrl*[如果以上链接无法点击,请将上面的地址复制到你的浏览器(如IE)的地址栏中打开.]（这是一封自动产生的email，请勿回复。）'
        );

        $this->getSettingService()->set('email', $default);
...
EOD;
    ...
    protected function _initCrontabJob()
    {
        ...
        $nineHour = 32400;
        $day = date('y-m-d', time());
        $sendTime = strtotime($day) + $nineHour;
        $this->getCrontabService()->createJob(array(
            'name' => 'SendNoticeMessageJob',
            'cycle' => 'everyday',
            'nextExcutedTime' => $sendTime,
            'jobClass' => 'CustomBundle\\Biz\\OthersRating\\Job\\SendNoticeMessageJob',
            'createdTime' => time(),
            'jobParams' => '',
        ));
    ...
    public function _initLdapUser()
    {
        $this->output->write('  初始化LDAP所有用户');
        $baseDn = ServiceKernel::instance()->getParameter('base_dn');
        $searchDn = ServiceKernel::instance()->getParameter('search_dn');
        $searchPassword = ServiceKernel::instance()->getParameter('search_password');
        $ldapHost = ServiceKernel::instance()->getParameter('ldap_host');
        $ldapPort = ServiceKernel::instance()->getParameter('ldap_port');
        $ldap = new LdapClient($ldapHost,$ldapPort);
        $ldap->bind($searchDn, $searchPassword);
        $ldapUsers = $ldap->find($baseDn, 'uid=*');
        if ($ldapUsers['count'] > 0) {
            for($i = 0; $i < $ldapUsers['count']; $i++) {
                $name = $ldapUsers[$i]['uid'][0];
                $truename = empty($ldapUsers[$i]['displayname'][0]) ? $name : $ldapUsers[$i]['displayname'][0];
                $mail = empty($ldapUsers[$i]['mail'][0]) ? $name.'@howzhi.com' : $ldapUsers[$i]['mail'][0];
                $employeeId = $ldapUsers[$i]['initials'][0];
                $user = array(
                    'email' => $mail,
                    'type' => 'ldap',
                    'nickname' => $name,
                ...
                $user = $this->getCustomUserService()->register($user, 'ldap');
                $this->getUserService()->updateUserProfile($user['id'], array('truename' => $truename));
            }
        }
     ...
     *
     */
    private function getCustomUserService()
    {
        return ServiceKernel::instance()->getBiz()->service('CustomBundle:User:UserService');
    }
    ...
    {
        return ServiceKernel::instance()->getBiz()->service('CustomBundle:Post:PostService');
    }
}
